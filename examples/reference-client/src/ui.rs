...
pub fn render(app: &mut App, frame: &mut Frame) {
    ...
    let messages_block = Block::default()
        ...
        .border_set(msg_border_set)
        .style(style);
    // Render chat list into the view
    let scrollbar =
        Scrollbar::new(ratatui::widgets::ScrollbarOrientation::VerticalRight).style(style);
    let sl = SubList {
...
impl<'a, 'b> StatefulWidget for SubList<'a, 'b> {
    ...
    fn render(self, area: Rect, buf: &mut ratatui::prelude::Buffer, state: &mut Self::State) {
        ...
        assert!(nb <= ne);
        let mut scrollbar = ScrollbarState::new(self.messages.len() - (ne - nb))
            .position(nb)
            .viewport_content_length(ne - nb);
        let new_area = Rect::new(0, 0, w, area.height + t as u16 + b as u16);
        let mut buffer = Buffer::empty(new_area);

        let items: Vec<String> = self.messages[nb..ne]
            .iter()
            .map(|x| x.to_string())
            .collect();
        ...
        // render scrollbar
        self.scrollbar
            .render(Rect::new(w, area.y, 1, area.height), buf, &mut scrollbar);
