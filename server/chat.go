...
func (s Server) handleChat(m *ClientMessage) {
...
func (st * GameState)CanChat(player uuid.UUID, chat ChatID) (bool, error) {
	...
	if !ok {return false, errors.New("Cannot find the given player")} // Invalid uuid

	ch, ok1 := st.Chats[chat] 
	if !ok1 {return false, errors.New("Cannot find target chat")} // Invalid uuid
	...
	for _, pog := range ch.CurrentAccess {
		...
		if pog.Group_name == GIDAny {return true, nil}
