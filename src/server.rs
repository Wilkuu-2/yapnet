...
impl<'a> Server<'_> { 
    ...
    pub fn handle_message(&mut self,m: Message) -> MessageResult {
        let cid = m.seq; 
        match m.data {
            MessageData::Hello { username } => {
                match self.state.new_user(&username) {
                    Ok(o) => {
                        self.users_connections.insert(cid as usize, username.clone()); 
                        o
                    } 
                    Err(o) => o 
                }
            },  
            MessageData::Back { token } => {
    ...
    pub fn auth_handle_message(&mut self, m: Message) -> MessageResult{
       if let Some(username) = self.users_connections.get(&(m.seq as usize)) {
            ...
            error_result("NotLoggedIn", "You need to be logged in to do this action.".to_string(), None)
      } 
    }
