...
use yapnet_core::{game::chat::MessageRef, prelude::Chat, protocol::message::{Message, MessageData}}; 
...
impl Client {
    ...
    async fn send_message_pre(&mut self, msg: MessageData) -> Result<(), Error> {
        ...
        self.writer.send(WSMessage::Text(to_string(&wrapped).expect("Serialization should never fail here"))).await.map_err(|e| Error::Websocket(e))
    }

    pub async fn send_register(&mut self, username: String) {
        self.send_message_pre(MessageData::Hello { username }).await.unwrap()
    }
    ...
    pub async fn send_login(&mut self, token: uuid::Uuid){
        self.send_message_pre(MessageData::Back { token }).await.unwrap();
    }
    ...
    fn handle_message(&mut self, msg: Message) -> ClientResult {
        let ret: ClientResultOuter = match msg.data {
            MessageData::Error { ref kind, info: _ , ref details } => { 
                ClientResultOuter(Ok(ClientAction::Error(format!("{} => {}", kind, details))), false)
            },
